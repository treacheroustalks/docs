...
Requirments from the customer specified that the system should consist of
...
fast.
It can be used to send javascript that updates only parts of the client page
...
by any node. The communication between the applications is shown in
...
sends a message to another one. An application calls a function in the API library
...
is the fact that the individual modules are very independent of each other. Even
though the architecture graphic shows a complete example of a running cluster,
we can choose to run some modules alone.
...
Messages in games never involve the user nickname for tactical reasons: if someone
remembers my nickname, she has an easier time to anticipate my moves since she
...
\subsection{HTTP}
The user goes to the landing page, there she finds a link to register which will
...
The exmpp library~\cite{exmpp} was used to do integration test through the XMPP
interface. Exmpp helped us to provide fully automatic and repeatable test
scenarios. Adding new functionality to our system was not completely done, until
its corresponding integration test was added.
