...
Otherwise, WebSocket is an amazing technology and it we think that it will have
a great impact on web development in the future as the tool support is increasing.
...
applications, the application calls a function in the API library,
...
One thing that is not immediately visible in our architecture (Fig.~\ref{fig:arch})is the fact that
...
and gameplay support for the user. We use the JavaScript library jQuery\cite{jquery}
...
allows us to quickly develop web pages. Bootstrap includes a CSS and HTML base for
...
Messages in-game never involves the user nickname for tactical reasons: if someone
remembers my nickname, he/she has an easier time to anticipate my moves since he
...
Each command starts with the command keyword and must end with an ``END'' tag.
...
The exmpp library\cite{exmpp} was used to do integration test through the XMPP interface.
