...
scaling where the system should be able to handle more load by simply adding more
nodes. No further explanations were provided regarding this feature, but since we
saw it as a challenge we invested a lot of work into it.
...
\begin{figure}[h]
 \centering
 \includegraphics[width=10cm]{./graphics/Modules_Communication.png}
...
to update the DOM
elements and make AJAX requests.
...
The simple ones without sessions are completely stateless and can handle requests
from any user.
...
``CAP controls''\cite{cap_controls} in riak-lingo) are very easy to tune and it is
quite well
...
There might be a conflict, if they want to play the same country or only one spot
is left.
...
where he is able to search for games, look at the games he is playing in (if any)
and
...
A full list of how to make plays can be found in appendix
...
\todo{Fill in what the Op interface can do}
